...
import { PrismaClient, UserRole, EquipmentLocation } from '@prisma/client'
...
export async function GET(request: Request) {
  ...
  const locationFilter = searchParams.get('location') as EquipmentLocation | null
...
export async function POST(request: Request) {
  ...
  if (!hasPermission) {
    return NextResponse.json({ error: 'Unauthorized to create equipment at this location' }, { status: 403 })
  }
...
function determineEquipmentLocation(data: any): EquipmentLocation {
  if (data.regionId) return EquipmentLocation.REGION_OFFICE
  if (data.countyId) return EquipmentLocation.COUNTY_OFFICE
  if (data.subCountyId) return EquipmentLocation.SUB_COUNTY_OFFICE
  return EquipmentLocation.NATIONAL_OFFICE
}