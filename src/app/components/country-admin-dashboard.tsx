...
import { countiesFetcher, fetcher, subCountiesFetcher } from "../utils/fetcher";
import {
  Card,
  CardHeader,
  CardTitle,
...
export const CountryAdminDashboard: React.FC = () => {
  ...
  const { data: countiesData, error: countiesError } = useSWR<PaginatedResponse<County>>(
    `/api/counties?page=${page}&limit=${limit}&search=${search}&sortBy=${sortBy}&order=${order}`,
  ...
  const { data: subCounties, error: subCountiesError } = useSWR<SubCounty[]>(
    expandedCounty ? `/api/subcounty?countyId=${expandedCounty}` : null,
    subCountiesFetcher
  );
  ...
  const handleCountyExpand = useCallback(
    (countyId: string) => {
      ...
      setExpandedSubCounty(null); // Reset sub-county when switching counties
    },
    []
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      {/* Counties and Sub-counties Section */}
      <section>
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle>Counties and Sub-counties</CardTitle>
            <CardDescription>Manage equipment by region</CardDescription>
                  ...
                  {countiesData.data.map((county) => (
                    <AccordionItem key={county.id} value={county.id}>
                      <AccordionTrigger
                        ...
                        className="text-lg font-medium text-white-800"
                      >
                        {county.name}
                      </AccordionTrigger>
                      <AccordionContent>
      ...
      {/* Add dialog component at the bottom of the return statement */}
      <EquipmentDialog
